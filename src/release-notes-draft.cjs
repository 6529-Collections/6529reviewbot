...
function formatReleaseNotesMarkdown(draft) {
  const lines = [
    ...
    "- provider-console readiness evidence has reviewed model availability, quota/rate-limit posture, provider-side spend caps or credit limits, billing alerts, and emergency key disablement;",
    "- provider keys and AWS credentials live only in bot-owned infrastructure;",
    "- IAM and secret custody evidence has reviewed OIDC trust scope, Data API resource scope, database grants, secret-store access principals, target-repo/browser secret exclusion, rotation owners, and break-glass revocation paths;",
    ...
    `- IAM and secret custody: ${TODO}`,
    `- Provider console readiness: ${TODO}`,
    ...
    `- \`npm run production:deployment-plan -- -- --host <production-bot-origin> --image <operator-registry>/6529reviewbot --operator-workspace <private-workspace-dir> --worker-dispatch-installation-id <central-repo-installation-id> --release v0.1.0 --require-ready\`: ${TODO}`,
