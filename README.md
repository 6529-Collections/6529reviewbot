...
- Redacts common token, sensitive-header, alert-webhook, AWS access-key id,
  AWS ARN, AWS account-id, and private-key shapes from worker, dispatch,
  ledger, alert, preflight, and repository-config diagnostics before they
  enter public or operator summaries.
...
- worker, dispatch, ledger, alert, preflight, and repository-config diagnostics
  redact common token, sensitive-header, alert-webhook, AWS access-key id,
  AWS ARN, AWS account-id, and private-key shapes before they are returned;
