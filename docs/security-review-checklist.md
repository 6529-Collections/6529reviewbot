...
- `npm run check:github-app-routes` passes so GitHub App browser handoff routes
  stay GET-only public-safe guidance surfaces and do not trigger review work.
...
- Worker diagnostic tails and GitHub API dispatch failure bodies redact common
  token, alert-webhook, AWS access-key id, and private-key shapes before they
  can enter queue results, run-control metadata, or logs.
