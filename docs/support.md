...
messages are still filtered for common secret-shaped values before JSON or
Markdown output. It also does not include target repository diffs, prompts,
...
to disclose. The support bundle redacts common secret-shaped values in git
output, but file names can still reveal project names, customer names, private
...
keys, GitHub tokens, bearer tokens, sensitive header values, AWS access keys,
alert webhook URLs, and private key blocks in public artifacts. It also flags
common local private paths, such as Windows private folders and Unix/macOS home
directories, so public examples use placeholders instead of operator-specific
filesystem locations. Shell command blocks also fail when they use the
reserved bot example host
