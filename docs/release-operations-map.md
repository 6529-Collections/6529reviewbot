...
handoff and dogfood ready-mode commands parse through their real CLIs with the
required private workspace, model price, worker-dispatch, strict preflight, and
ready-mode inputs. The check is included in `npm run release:check`.
...
linked from `iam-and-secrets` operator evidence and production cutover
evidence.
...
operator-workspace, strict-preflight, and model-price-file ready-mode gates,
private workspace markers, model price coverage, baseline provider/IAM handoff
guidance, Markdown redaction, source invariants, and docs aligned.
`npm run check:dogfood-go-live` keeps the final dogfood traffic packet's
operator-workspace, strict-preflight, and model-price-file ready-mode gates,
private workspace markers, baseline provider/IAM handoff guidance, Markdown
...
policy evidence, IAM and secret custody evidence, model-pricing evidence,
provider-console-readiness evidence, alert delivery-plan evidence, dashboard
...
disclosure evidence, IAM and secret custody evidence, private admin auth
...
fields, including provider console readiness and IAM/secret custody, deferral
handling, vague or failed validation evidence, public-safety redaction,
...
- Use `npm run operator:drill -- -- --dir <private-workspace-dir>` as the
  public-safe rehearsal before the production deployment, dashboard
  deployment, alert delivery, and final release-candidate, dogfood promotion,
  and go-live `--require-ready` commands.
