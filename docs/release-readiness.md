...
- release operations map validation parses final production handoff and
  dogfood ready-mode commands through the real CLIs so required private
  workspace, model price, worker-dispatch, strict preflight, and ready-mode
  flags cannot drift;
...
final production handoff commands, and final dogfood ready-mode CLI
requirements.
`npm run check:release-notes` is included in `npm run release:check` and keeps
